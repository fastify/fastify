...
The main difference between these errors are that `System` errors are handled by fastify and result in an [`unhandled rejection`](#unhandled-rejections).
...
In Node.js it's very common to derive from the `Error` type to build your own error hierachy. If you looking for a solution to work with http status codes we recommend [http-errors](https://github.com/jshttp/http-errors).
