...
Starting from the `User Handler`, the user can call `reply.hijack()` and prevent fastify from sending the response and running of the `preSerialization`, `onSend`, `onError`, `onTimeout`, and `onResponse` hooks.
