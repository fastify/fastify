...
```
**Note**: The body cannot serialize inside `req` method, because the request is serialized when we create the child logger. At that time, the body is not yet parsed.
