...
- [Errors](#errors)
  ...
  - [Fastify Error Codes](#fastify-error-codes)
    ...
    - [FST_ERR_LOG_INVALID_LOG_DISPATCHER](#fst_err_log_invalid_log_dispatcher)
    - [FST_ERR_LOG_INVALID_LOG_DISPATCHER_FN](#fst_err_log_invalid_log_dispatcher_fn)
...
| <a id="fst_err_log_invalid_log_dispatcher">FST_ERR_LOG_INVALID_LOG_DISPATCHER</a> | The `logDispatcher` option must be an object with valid method overrides. | Provide an object whose keys are valid `LogDispatcher` method names. | - |
| <a id="fst_err_log_invalid_log_dispatcher_fn">FST_ERR_LOG_INVALID_LOG_DISPATCHER_FN</a> | A `logDispatcher` override must be a function. | Use a function for each overridden method. | - |
