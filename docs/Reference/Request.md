...
- [.validateInput(data, schema | httpPart, [httpPart])](#validate) -
  Validates the input using the specified schema and returns `true` if the
  input is valid, `false` otherwise. If `httpPart` is provided, the function
  uses the validator for that HTTP part. Defaults to `null`.
