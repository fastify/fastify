...
<a name="server"></a>
### .server
The Fastify server instance, scoped to the current [encapsulation context](./Referenence/Encapsulation.md).
...
### .callNotFound()
Invokes the custom not found handler. Note that it will only call `preHandler` hook specified in [`setNotFoundHandler`](./Referenence/Server.md#set-not-found-handler).
...
If you want to customize error handling, check out [`setErrorHandler`](./Referenence/Server.md#seterrorhandler) API.<br>
...
The not found errors generated by the router will use the  [`setNotFoundHandler`](./Referenence/Server.md#setnotfoundhandler)
