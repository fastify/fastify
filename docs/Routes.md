...
* `exposeHeadRoute`: creates a sibling `HEAD` route from the current one. Only applies for `GET` routes. Defaults to the value of [`exposeHeadRoutes`](Server.md#exposeHeadRoutes) instance option.
