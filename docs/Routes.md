...
Combining different styles like `callback-style` and `async-await` might become hard and error prone. We recommend to use just one style within an application to ensure all errors are handled consistently.
