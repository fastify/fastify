...
So far, our routes have been taking whatever the client sends and returning 
whatever reply body we produce. This is risky and inefficient.
...
We won’t dive too deep into here, because it would requires encapsulation and
lifecycle hooks
knwoledges, but it’s useful to know where validation and serialization live in
the internals.

At the core, Fastify builds a `SchemaController` (see `lib/schema-controller.js`)
for every context. 

This controller wires together:

* **Validator compiler** - usually Ajv, set up via `setValidatorCompiler`
* **Serializer compiler** - usually fast-json-stringify, set up via `setSerializerCompiler`

On the request side (`lib/request.js`), the `Request` object has helpers like
`compileValidationSchema` and `validateInput`. 
These lazily compile a schema into a validation function, caching the result.

On the reply side (`lib/reply.js`), the `Reply` object mirrors this with 
`compileSerializationSchema` and `serializeInput`, again caching compiled
serializers.
