...
const annotateError = require('./errors').annotateErrorWithCode
...
ContentTypeParser.prototype.run = function (contentType, handler, request, reply) {
  ...
  if (parser === undefined) {
    var err = new Error('Unsupported Media Type: ' + contentType)
    err = annotateError(err, errorCodes.FST_ERR_HTTP)
    ...
    if (parser.asString === true || parser.asBuffer === true) {
      ...
      if (result && typeof result.then === 'function') {
        result.then(body => done(null, body)).catch((err) => {
          err = annotateError(err, errorCodes.FST_ERR_CONTENT_TYPE_REJECTION)
          done(err)
        })
      }
    }
  ...
  function done (error, body) {
    if (error) {
      if (error.code === undefined) {
        error = annotateError(error, errorCodes.FST_ERR_CONTENT_TYPE)
      }
      reply.send(error)
    } else {
      request.body = body
...
function rawBody (request, reply, options, parser, done) {
  ...
  function onEnd (err) {
    ...
    if (err !== undefined) {
      reply.code(400).send(annotateError(err, errorCodes.FST_ERR_REQUEST_STREAM))
      return
    }

    if (!Number.isNaN(contentLength) && receivedLength !== contentLength) {
      var contentLenghtErr = new Error('Request body size did not match Content-Length')
      contentLenghtErr = annotateError(err, errorCodes.FST_ERR_HTTP_CONTENT_LENGTH)
    ...
    if (result && typeof result.then === 'function') {
      result.then(body => done(null, body), err => {
        err = annotateError(err, errorCodes.FST_ERR_RAW_CONTENT_TYPE_REJECTION)
        done(err)
      })
    }
  }
...
function defaultJsonParser (req, body, done) {
  try {
    ...
    err.statusCode = 400
    return done(annotateError(err, errorCodes.FST_ERR_DEFAULT_TYPE_PARSER), undefined)
  }
  done(null, json)
}
