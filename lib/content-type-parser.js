...
function ContentTypeParser (bodyLimit, onProtoPoisoning, onConstructorPoisoning, warnings) {
...
ContentTypeParser.prototype.add = function (contentType, opts, parserFn) {
  ...
  if (contentType === '*') {
    ...
    if (contentTypeIsString) {
      ...
      validateRegExp.call(this, contentType)
...
function buildContentTypeParser (c) {
  const contentTypeParser = new ContentTypeParser(undefined, undefined, undefined, c.warnings)
...
function validateRegExp (regexp) {
  ...
  if (regexp.source[0] !== '^' && regexp.source.includes(';?') === false) {
    this.warnings.emit('FSTSEC001', regexp.source)
  }
}
