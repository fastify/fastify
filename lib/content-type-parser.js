...
function ContentTypeParser (bodyLimit, onProtoPoisoning, onConstructorPoisoning, contentTypeHeaderParserFactory) {
  this.parse = ContentType.from
  if (contentTypeHeaderParserFactory !== undefined) {
    this.parse = contentTypeHeaderParserFactory(this.parse)
    if (typeof this.parse !== 'function') {
      throw new TypeError('contentTypeHeaderParserFactory must return a function')
    }
  }
...
ContentTypeParser.prototype.add = function (contentType, opts, parserFn) {
  ...
  if (contentType === '*') {
    ...
    if (contentTypeIsString) {
      const ct = this.parse(contentType)
      if (ct.isValid === false) {
        throw new FST_ERR_CTP_INVALID_TYPE()
      }
...
ContentTypeParser.prototype.hasParser = function (contentType) {
  if (typeof contentType === 'string') {
    contentType = this.parse(contentType).toString()
  } else {
...
ContentTypeParser.prototype.existingParser = function (contentType) {
  if (typeof contentType === 'string') {
    const ct = this.parse(contentType).toString()
...
ContentTypeParser.prototype.getParser = function (contentType) {
  if (typeof contentType === 'string') {
    ...
    contentType = this.parse(contentType)
  }
...
function buildContentTypeParser (c) {
  ...
  contentTypeParser.parse = c.parse
