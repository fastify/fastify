...
function ContentTypeParser (bodyLimit) {
  ...
  this.customParsers = { 'application/json': new Parser(true, false, bodyLimit, this.defaultJsonParser) }
...
ContentTypeParser.prototype.hasParser = function (contentType) {
  if (contentType === 'application/json') {
    return this.customParsers['application/json'].fn !== this.defaultJsonParser
  }
...
function defaultJsonParser () {
  ...
  return function jitsonParser (req, body, done) {
    try {
      var json = parse(body)
    } catch (err) {
      err.statusCode = 400
