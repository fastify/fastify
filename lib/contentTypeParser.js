...
ContentTypeParser.prototype.run = function (contentType, handler, request, reply) {
  ...
  } else if (reply.context.preDecoding !== null) {
    onPredecodingHookRunner(reply.context.preDecoding, request, reply, request.raw, processDecodedBody)
  } else {
    processDecodedBody(null, request.raw)
  }

  function processDecodedBody (error, rawRequestBody) {
    if (error) {
      done(error)
      return
    }
    ...
    if (parser.asString === true || parser.asBuffer === true) {
      ...
      if (parser.isDeprecatedSignature) {
        result = parser.fn(rawRequestBody, done)
      } else {
        result = parser.fn(request, rawRequestBody, done)
      }
...
function onPredecodingHookRunner (functions, request, reply, raw, cb) {
  let i = 0

  function next (err, newRaw) {
    if (reply.sent) {
      return
    }
    ...
    const result = fn(request, reply, newRaw, next)
...
function rawBody (request, reply, rawBody, options, parser, done) {
  ...
  if (asString === true) {
    rawBody.setEncoding('utf8')
  }

  rawBody.on('data', onData)
  rawBody.on('end', onEnd)
  rawBody.on('error', onEnd)
  rawBody.resume()

  function onData (chunk) {
    receivedLength += chunk.length

    if ((rawBody.receivedEncodedLength || receivedLength) > limit) {
      rawBody.removeListener('data', onData)
      rawBody.removeListener('end', onEnd)
      rawBody.removeListener('error', onEnd)
      reply.send(new FST_ERR_CTP_BODY_TOO_LARGE())
      return
    }
  ...
  function onEnd (err) {
    ...
    if (!Number.isNaN(contentLength) && (rawBody.receivedEncodedLength || receivedLength) !== contentLength) {
