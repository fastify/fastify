...
function getDefaultJsonParser (onProtoPoisoning, onConstructorPoisoning) {
  return (req, body, done) => defaultJsonParser(req, body, done, { onProtoPoisoning, onConstructorPoisoning })
}

function defaultJsonParser (req, body, done, opts = {}) {
  if (body === '' || body == null) {
    return done(new FST_ERR_CTP_EMPTY_JSON_BODY(), undefined)
  }

  try {
    var json = secureJson.parse(body, { protoAction: opts.onProtoPoisoning, constructorAction: opts.onConstructorPoisoning })
  } catch (err) {
    err.statusCode = 400
