...
ContentTypeParser.prototype.run = function (contentType, handler, request, reply) {
  ...
  // We must allocate a new AsyncResource otherwise the context will be lost
  // when using the AsyncLocalStorage.run() API.
...
function _done (error, body) {
  if (error) {
    ...
    this.reply.request.body = body
    const handler = this.handler
    handler(this.reply.request, this.reply)
  }
}
