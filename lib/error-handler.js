  ...
  if (reply[kReplyNextErrorHandler] === false) {
    fallbackErrorHandler(error, reply, function (reply, payload) {
      try {
        ...
        reply.server.internalLogger.errorOnWriteHead(reply.log, error, reply)
...
function defaultErrorHandler (error, request, reply) {
  ...
  if (reply.statusCode < 500) {
    reply.server.internalLogger.response4xxError(reply.log, error, reply)
  } else {
    reply.server.internalLogger.response5xxError(reply.log, error, request, reply)
  }
  reply.send(error)
}
...
function fallbackErrorHandler (error, reply, cb) {
  ...
  try {
    ...
    reply.server.internalLogger.errorOnSerialization(reply.log, err, res)
