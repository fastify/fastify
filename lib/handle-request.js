...
function handleRequest (err, request, reply) {
  ...
  if (this[kSupportedHTTPMethods].bodywith.has(method)) {
    ...
    const hasInvalidContentType = request[kRequestContentType].isValid === false
    if (hasInvalidContentType && request[kRouteContext].contentTypeParser.strictContentTypeHeaderValidation) {
      reply[kReplyIsError] = true
      reply.status(415).send(new FST_ERR_CTP_INVALID_MEDIA_TYPE())
      return
    }
    const contentType = hasInvalidContentType ? ctHeader : request[kRequestContentType].toString()
