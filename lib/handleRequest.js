...
function preValidationCallback (err, request, reply) {
  ...
  let validationErr
  try {
    // TODO: Consider using Promise.try when Node.js 20 support is dropped (requires Node.js 22.10+)
    validationErr = validateSchema(reply[kRouteContext], request)
  } catch (thrownValidationErr) {
    // Ensure synchronous validation errors are always routed through Fastify's error handling
    // Use validationCompleted to properly handle attachValidation option
    validationCompleted(request, reply, thrownValidationErr)
    return
  }
