...
function handleRequest (req, res, params, store) {
  ...
  if (method === 'POST' || method === 'PUT' || method === 'PATCH') {
    ...
    setImmediate(wrapReplyEnd, req, res, store, 415)
    return
  }
  ...
  setImmediate(wrapReplyEnd, req, res, store, 405)
  return
}
...
function jsonBodyParsed (err, body, req, res, params, handle) {
  if (err) {
    setImmediate(wrapReplyEnd, req, res, handle, 422)
    return
  }
...
function handler (store, params, req, res, body, query, headers) {
  ...
  if (valid !== true) {
    setImmediate(wrapReplyEnd, req, res, store, 400, valid)
    return
  }
...
function wrapReplyEnd (req, res, store, statusCode, payload) {
  const reply = new Reply(req, res, store)
