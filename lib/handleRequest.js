...
function handleRequest (req, res, params, context) {
  ...
  var err = annotateError(new Error('Not Found'), errorCodes.FST_ERR_HTTP)
...
function preHandlerCallback (err, reply) {
  ...
  if (err) {
    reply.send(annotateError(err, errorCodes.FST_ERR_PRE_HANDLER_HOOK))
    return
  }
  ...
  if (result && typeof result.then === 'function') {
    result
      ...
      .catch((err) => {
        reply.sent = false
        reply._isError = true
        err = annotateError(err, errorCodes.FST_ERR_HANDLER_REJECTION)
        reply.send(err)
      })
  }
}
