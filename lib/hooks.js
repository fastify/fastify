...
function buildHooks (h) {
  ...
  hooks.onCompilerReady = []
  return hooks
}

function hookRunnerApplication (hookName, server, cb) {
  const hooks = server[kHooks][hookName]
  var i = 0
  var c = 0

  function next (err) {
    if (err || (i === hooks.length && c === server[kChildren].length)) {
      cb(err)
      return
    }
    ...
    if (i === hooks.length) {
      ...
      hookRunnerApplication(hookName, child, next)
      return
    }

    const result = hooks[i++].call(server, next)
