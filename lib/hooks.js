...
function onListenHookRunner (boot, server) {
  ...
  function wrap (fn, server) {
    return function (err, done) {
      ...
      function onListenPromisify (fn) {
        return async function () {
          return new Promise((resolve, reject) => {
            try {
              fn = fn.bind(server, resolve)
              fn()
            } catch (e) {
              reject(e)
            }
      ...
      let promisified = fn
      if (fn.length === 1) {
        promisified = onListenPromisify(fn).bind(server)
      }
      try {
        const ret = promisified.call(server)
        if (ret && typeof ret.catch === 'function') {
          ret.catch(err => server.log.error(err))
        }
      } catch (error) {
        err = error
        server.log.error(err)
