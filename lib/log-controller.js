...
const { kRouteContext } = require('./symbols')
...
class LogController {
  ...
  /**
   * Logs an incoming request at the route's `requestLogLevel`.
   *
  ...
  incomingRequest (request, reply, metadata) {
    ...
    logAtLevel(request.log, getRequestLogLevel(request), { req: request }, 'incoming request')
  }

  /**
   * Logs the outcome of a completed request.
   * Uses `error` level when an error is present and the route's
   * `requestLogLevel` otherwise.
   *
...
function getRequestLogLevel (request) {
  return request[kRouteContext]?.requestLogLevel || 'info'
}

function logAtLevel (logger, level, data, message) {
