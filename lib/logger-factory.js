...
const {
  ...
  FST_ERR_LOG_INVALID_LOG_DISPATCHER,
  FST_ERR_LOG_INVALID_LOG_DISPATCHER_FN
} = require('./errors')
const { LogDispatcher, overridableMethods } = require('./log-dispatcher.js')
...
function createChildLogger (context, logger, req, reqId, loggerOpts) {
  const loggerBindings = {
    [context.server.logDispatcher.requestIdLogLabel]: reqId
  }
...
function createLogDispatcher (options, overrides) {
  ...
  if (typeof overrides !== 'object' || Array.isArray(overrides)) {
    throw new FST_ERR_LOG_INVALID_LOG_DISPATCHER(overridableMethods.join(', '), typeof overrides)
  }

  const invalidKeys = Object.keys(overrides).filter(k => !overridableMethods.includes(k))
  if (invalidKeys.length) {
    throw new FST_ERR_LOG_INVALID_LOG_DISPATCHER(invalidKeys.join(', '))
  }

  for (const key in overrides) {
    if (typeof overrides[key] !== 'function') {
      throw new FST_ERR_LOG_INVALID_LOG_DISPATCHER_FN(key)
    }
    dispatcher[key] = overrides[key]
  }
