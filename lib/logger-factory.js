...
const { LogDispatcher } = require('./log-dispatcher.js')
...
function createChildLogger (context, logger, req, reqId, loggerOpts) {
  const loggerBindings = {
    [context.server[kLogDispatcher].requestIdLogLabel]: reqId
  }
...
function createLogDispatcher (options) {
  const userDispatcher = options.logDispatcher
  if (!userDispatcher) {
    return new LogDispatcher(options)
  }

  if (userDispatcher instanceof LogDispatcher) {
    return userDispatcher
  }

  throw new FST_ERR_LOG_INVALID_LOG_DISPATCHER(typeof userDispatcher)
}
