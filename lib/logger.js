 ...
 */
function createChildLogger (fastify, logger, req, reqId, options, loggerOpts) {
  const loggerBindings = {
    [options.requestIdLogLabel]: reqId
  }
  const child = options.childLoggerFactory.call(fastify, logger, loggerBindings, loggerOpts || {}, req)

  // Optimisation: bypass validation if the factory is our own default factory
  if (options.childLoggerFactory !== defaultChildLoggerFactory) {
