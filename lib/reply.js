...
Reply.prototype.send = function (payload) {
  ...
  if (payload instanceof Error || this[kReplyIsError] === true) {
    // If the reply was already hijacked, we skip sending the error since
    // the caller is expected to handle it depending on their use-case
    if (this[kReplyHijacked] === true) {
      return this
    }
  ...
  // If the reply was hijacked, we skip sending
  if (this[kReplyHijacked] === true) {
    return this
  }
...
function handleError (reply, error, cb) {
  // We skip sending the error if the reply was hijacked.
  // We want onError hooks to run if an error was thrown before the reply was hijacked,
  // having this check here allows the caller to use an onError hook to process the error and
  // to hijack the reply (preventing the error from being sent using this function)
