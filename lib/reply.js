...
function onSendEnd (reply, payload) {
  ...
  // before continuing
  const isResponse = payload != null && typeof payload === 'object' && toString.call(payload) === '[object Response]'
  if (isResponse) {
  ...
  removeHttp2InvalidHeaders(reply)
...
function removeHttp2InvalidHeaders (reply) {
  if (!isHttp2Reply(reply)) return

  // RFC 9113 section 8.2.2 forbids connection-specific fields in HTTP/2.
  // HTTP2-Settings is also only valid during an HTTP/1.1 h2c upgrade.
  for (const header of HTTP2_FORBIDDEN_HEADERS) {
  ...
  if (te !== undefined && !isValidHttp2Te(te)) {
    reply.removeHeader('te')
  }
}

function isValidHttp2Te (value) {
...
function sendTrailer (payload, res, reply) {
  ...
  for (const trailerName of trailerHeaders) {
    ...
    function cb (err, value) {
      ...
      // Trailer errors happen after the response headers are sent,
      // so log them at debug level and omit the invalid trailer.
      if (err) {
        reply.log.debug(err)
      } else {
        try {
          validateHeaderValue(trailerName, value)
    ...
    let result
    try {
      result = reply[kReplyTrailers][trailerName](reply, payload, cb)
    } catch (err) {
      cb(err)
      continue
    }
