...
Reply.prototype.getSerializationFunction = function (schemaOrStatus, contentType) {
  ...
  if (typeof schemaOrStatus === 'string' || typeof schemaOrStatus === 'number') {
    ...
    serialize = this[kRouteContext][kReplyCacheSerializeFns]?.get(schemaOrStatus)?.first.value
  }
...
Reply.prototype.compileSerializationSchema = function (schema, httpStatus = null, contentType = null) {
  ...
  // The default compiler ignores httpStatus and contentType, so one fn per
  // schema is enough. A custom compiler may use them, so they join the key.
  const cacheKey = this[kRouteContext].serializerCompiler != null ||
    this.server[kSchemaController].isCustomSerializerCompiler
