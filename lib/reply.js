...
Object.defineProperties(Reply.prototype, {
  server: {
    get () {
      return this.request[kContext].server
    }
  },
  sent: {
    enumerable: true,
...
Reply.prototype.getSerializationFunction = function (schemaOrStatus) {
  ...
  if (typeof schemaOrStatus === 'string' || typeof schemaOrStatus === 'number') {
    serialize = this[kContext][kSchemaResponse]?.[schemaOrStatus]
  } else if (typeof schemaOrStatus === 'object') {
    serialize = this[kContext][kReplySerializeWeakMap]?.get(schemaOrStatus)
  }
...
Reply.prototype.compileSerializationSchema = function (schema, httpStatus = null) {
  ...
  if (this[kContext][kReplySerializeWeakMap]?.has(schema)) {
    return this[kContext][kReplySerializeWeakMap].get(schema)
  }

  const serializerCompiler = this[kContext].serializerCompiler ||
  ...
  if (this[kContext][kReplySerializeWeakMap] == null) {
    this[kContext][kReplySerializeWeakMap] = new WeakMap()
  }

  this[kContext][kReplySerializeWeakMap].set(schema, serializeFn)
...
Reply.prototype.serializeInput = function (input, schema, httpStatus) {
  ...
  if (httpStatus != null) {
    serialize = this[kContext][kSchemaResponse]?.[httpStatus]

    if (serialize == null) throw new FST_ERR_MISSING_SERIALIZATION_FN(httpStatus)
  } else {
    // Check if serialize function already compiled
    if (this[kContext][kReplySerializeWeakMap]?.has(schema)) {
      serialize = this[kContext][kReplySerializeWeakMap].get(schema)
    } else {
...
Reply.prototype.serialize = function (payload) {
  if (this[kReplySerializer] !== null) {
    ...
    if (this[kContext] && this[kContext][kReplySerializerDefault]) {
      return this[kContext][kReplySerializerDefault](payload, this.raw.statusCode)
    } else {
      return serialize(this[kContext], payload, this.raw.statusCode)
    }
  }
}
...
function preserializeHook (reply, payload) {
  if (reply[kContext].preSerialization !== null) {
    onSendHookRunner(
      reply[kContext].preSerialization,
      reply.request,
      reply,
      payload,
      preserializeHookEnd
...
function preserializeHookEnd (err, request, reply, payload) {
  ...
  try {
    ...
    } else if (reply[kContext] && reply[kContext][kReplySerializerDefault]) {
      payload = reply[kContext][kReplySerializerDefault](payload, reply.raw.statusCode)
    } else {
      payload = serialize(reply[kContext], payload, reply.raw.statusCode)
    }
  } catch (e) {
...
function wrapSeralizationError (error, reply) {
  error.serialization = reply[kContext].config
}

function onSendHook (reply, payload) {
  if (reply[kContext].onSend !== null) {
    onSendHookRunner(
      reply[kContext].onSend,
      reply.request,
      reply,
      payload,
      wrapOnSendEnd
...
function onErrorHook (reply, error, cb) {
  if (reply[kContext].onError !== null && !reply[kReplyNextErrorHandler]) {
    reply[kReplyIsRunningOnErrorHook] = true
    onSendHookRunner(
      reply[kContext].onError,
      reply.request,
      reply,
      error,
...
function notFound (reply) {
  if (reply[kContext][kFourOhFourContext] === null) {
    reply.log.warn('Trying to send a NotFound error inside a 404 handler. Sending basic 404 response.')
    reply.code(404).send('404 Not Found')
    return
  }

  reply.request[kContext] = reply[kContext][kFourOhFourContext]

  // preHandler hook
  if (reply[kContext].preHandler !== null) {
    hookRunner(
      reply[kContext].preHandler,
      hookIterator,
      reply.request,
      reply,
