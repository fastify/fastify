...
Reply.prototype.hijack = function () {
  ...
  // Clear handler timeout — hijacked replies manage their own lifecycle
  if (this.request._timeoutTimer) {
    clearTimeout(this.request._timeoutTimer)
    this.request._timeoutTimer = null
  }
  if (this.request._onAbort) {
    this.request.raw.removeListener('close', this.request._onAbort)
    this.request._onAbort = null
  }
  return this
}
...
function setupResponseListeners (reply) {
  ...
  const onResFinished = err => {
    ...
    // Clean up handler timeout resources
    if (reply.request._timeoutTimer) {
      clearTimeout(reply.request._timeoutTimer)
      reply.request._timeoutTimer = null
    }
    if (reply.request._onAbort) {
      reply.request.raw.removeListener('close', reply.request._onAbort)
      reply.request._onAbort = null
    }
