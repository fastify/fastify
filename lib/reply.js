...
// RFC 9110: 1xx, 204 and 205 cannot contain trailers. 304 cannot contain a
// message body, and HTTP/1.1 trailers require chunked transfer encoding.
function statusCannotHaveTrailers (statusCode) {
...
function applyTrailerHeaders (reply) {
  if (reply[kReplyTrailers] === null) {
    return
  }

  if (statusCannotHaveTrailers(reply.raw.statusCode)) {
  ...
  if (header !== '') {
    ...
    // RFC 9110 §6.1: MUST NOT send Content-Length with Transfer-Encoding
...
function onSendEnd (reply, payload) {
  ...
  // Apply trailers after the status is known so bodyless statuses (204/304/…)
  // do not send Transfer-Encoding: chunked, which Node rejects with
  // ERR_HTTP_TRAILER_INVALID.
  applyTrailerHeaders(reply)
