...
const onSendHookRunner = require('./hookRunner').onSendHookRunner
...
Reply.prototype._setup = function (hasLogger) {
  if (hasLogger === false && this.context.onResponse === null) return

  this._startTime = now()

  var onResFinished = err => {
    this.res.removeListener('finish', onResFinished)
    this.res.removeListener('error', onResFinished)
    ...
    } else if (hasLogger === true) {
      onResponseCallback(err, this)
    }
  }

  this.res.on('finish', onResFinished)
  this.res.on('error', onResFinished)
}
