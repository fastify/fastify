  ...
  if (this[kReplySerializer] !== null && typeof payload === 'string' && payload.length) {
    if (contentType && contentType.indexOf('application/json') !== -1) {
      ...
      if (!(payload[0] === '"' && payload[payload.length - 1] === '"')) {
        ...
        alreadyStringified = true
      }
    }
  }

  // Run serializer if it exists and payload is not string or not yet stringified
  if (this[kReplySerializer] !== null) {
    if (typeof payload !== 'string' || !alreadyStringified) {
      preSerializationHook(this, payload)
      return this
    }
    // Payload already stringified, do not run serializer again
    ...
    if (!hasContentType) {
      ...
      this[kReplyHeaders]['content-type'] = trimmedContentType.endsWith(';')
        ? `${trimmedContentType} charset=utf-8`
        : `${trimmedContentType}; charset=utf-8`
    }

    // If payload is not string, run preSerializationHook
