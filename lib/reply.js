...
function logStreamError (logger, err, reply, res) {
  if (err.code === 'ERR_STREAM_PREMATURE_CLOSE') {
    reply.server.internalLogger.streamClosedPrematurely(logger, res)
  } else {
    reply.server.internalLogger.responseTerminated(logger, err)
  }
}
...
function sendWebStream (payload, res, reply) {
  ...
  eos(res, function (err) {
    if (sourceOpen) {
      if (err != null && res.headersSent && !errorLogged) {
        errorLogged = true
        logStreamError(reply.log, err, reply, res)
      }
  ...
  function onReadError (err) {
    ...
    if (res.headersSent || reply.request.raw.aborted === true) {
      if (!errorLogged) {
        errorLogged = true
        logStreamError(reply.log, err, reply, reply)
      }
      res.destroy()
    } else {
      onErrorHook(reply, err)
...
function onResponseCallback (err, request, reply) {
  if (err != null) {
    reply.server.internalLogger.responseError(reply.log, err, reply)
    return
  }

  reply.server.internalLogger.responseSuccess(reply.log, reply)
}
