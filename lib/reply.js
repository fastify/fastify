...
Reply.prototype.getSerializationFunction = function (schemaOrStatus) {
  ...
  if (typeof schemaOrStatus === 'string' || typeof schemaOrStatus === 'number') {
    serialize = this.context[kSchemaResponse] != null
      ? this.context[kSchemaResponse][schemaOrStatus]
      : null
  } else if (typeof schemaOrStatus === 'object') {
    serialize = this.context[kRequestSerializeFns].get(schemaOrStatus)
  }
...
Reply.prototype.compileSerializationSchema = function (schema, httpStatus) {
  ...
  if (this.context[kRequestSerializeFns].has(schema)) {
    ...
    if (serializerCompiler == null) {
      this.server[kSchemaController].setupSerializer(
        this.server[kOptions]
      )
      this.context.config.serializerCompiler = this.server[
        kSchemaController
      ].serializerCompiler
    }

    serialize = this.context.config.serializerCompiler({
      schema,
      method,
      url,
      httpStatus
...
Reply.prototype.serializeInput = function (input, schema, httpStatus) {
  ...
  if (httpStatus != null) {
    serialize = this.context[kSchemaResponse] != null
      ? this.context[kSchemaResponse][httpStatus]
      : null
    ...
    if (this.context[kRequestSerializeFns].has(schema)) {
      ...
      serialize = this.compileSerializerSchema(schema, httpStatus)
    }
  }
