...
Reply.prototype.getSerializationFunction = function (schemaOrStatus, contentType) {
  ...
  if (typeof schemaOrStatus === 'string' || typeof schemaOrStatus === 'number') {
    ...
    // The cache is keyed by schema then by "httpStatus:contentType".
    // getSerializationFunction receives no metadata, so look up the no-metadata key.
    serialize = this[kRouteContext][kReplyCacheSerializeFns]?.get(schemaOrStatus)?.get(':')
  }
...
Reply.prototype.compileSerializationSchema = function (schema, httpStatus = undefined, contentType = undefined) {
  ...
  // Cache is two-level: WeakMap<schema, Map<"httpStatus:contentType", serializeFn>>
  // so that the same schema object compiled with different metadata produces
  // distinct serializers (fixes #6987) and omitted metadata is represented as
  // undefined rather than null (fixes #6988).
  const metaKey = `${httpStatus ?? ''}:${contentType ?? ''}`
  const existing = this[kRouteContext][kReplyCacheSerializeFns]?.get(schema)
  if (existing !== undefined) {
    const cached = existing.get(metaKey)
    if (cached !== undefined) {
      return cached
    }
  }
  ...
  let metaMap = this[kRouteContext][kReplyCacheSerializeFns].get(schema)
  if (metaMap === undefined) {
    metaMap = new Map()
    this[kRouteContext][kReplyCacheSerializeFns].set(schema, metaMap)
  }
  metaMap.set(metaKey, serializeFn)
...
Reply.prototype.serializeInput = function (input, schema, httpStatus, contentType) {
  ...
  if (httpStatus != null) {
    ...
    // Check if serialize function already compiled
    const metaKey = `${httpStatus ?? ''}:${contentType ?? ''}`
    const cached = this[kRouteContext][kReplyCacheSerializeFns]?.get(schema)?.get(metaKey)
    if (cached !== undefined) {
      serialize = cached
    } else {
