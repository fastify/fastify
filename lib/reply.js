...
function onSendEnd (err) {
  if (err) {
    return handleError(this.reply, err)
  }
  if (!this.reply.res.getHeader('Content-Length')) {
...
function handleError (reply, err) {
  ...
  var store = reply.store
  var errorHandler = store && store.errorHandler
