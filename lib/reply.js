  ...
  sent: {
    ...
    set (value) {
      ...
      if (this.sent && this[kReplyHijacked]) {
        throw new FST_ERR_REP_ALREADY_SENT(this.request.url)
      }
...
Reply.prototype.send = function (payload) {
  ...
  if (this.sent) {
    this.log.warn({ err: new FST_ERR_REP_ALREADY_SENT(this.request.url) })
    return this
  }
...
function safeWriteHead (reply, statusCode) {
  ...
  try {
    ...
    if (err.code === 'ERR_HTTP_HEADERS_SENT') {
      reply.log.warn(`The reply already sent, did you forget to "return reply" in the "${reply.request.raw.url}" route?`)
    }
    throw err
  }
}
