...
const {
  ...
  FST_ERR_BAD_HEADER_NAME,
  FST_ERR_BAD_HEADER_VALUE,
...
const INVALID_HEADER_NAME_REGEX = /[^!#$%&'*+\-.^_`|~\da-zA-Z]/

Reply.prototype.header = function (key, value = '') {
  if (typeof key !== 'string' || key.length === 0 || INVALID_HEADER_NAME_REGEX.test(key)) {
    throw new FST_ERR_BAD_HEADER_NAME(key || String(key))
  }

  if (typeof value === 'string') {
    if (INVALID_HEADER_CHAR_REGEX.test(value)) {
      throw new FST_ERR_BAD_HEADER_VALUE(key)
    }
  } else if (Array.isArray(value)) {
    for (let i = 0; i < value.length; i++) {
      if (typeof value[i] === 'string' && INVALID_HEADER_CHAR_REGEX.test(value[i])) {
        throw new FST_ERR_BAD_HEADER_VALUE(key)
      }
    }
  }
