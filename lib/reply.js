...
function handleReplyEnd (reply, payload) {
  if (reply.store.onSend.length === 0) {
    return onSendEnd(null, reply, payload)
  }
  setImmediate(
    runHooks,
...
function wrapOnSendEnd (err) {
  onSendEnd.call(this.reply.store.fastify, err, this.reply, this.payload)
}

function onSendEnd (err, reply, payload) {
  if (err) {
...
function hookIterator (fn, cb) {
  fn.call(this.reply.store.fastify, this.request, this.reply, this.payload, cb)
}
