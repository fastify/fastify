...
function logStreamError (loggerInstance, err, res) {
  if (err.code === 'ERR_STREAM_PREMATURE_CLOSE') {
    if (!loggerInstance[kDisableRequestLogging]) {
      loggerInstance.info({ res }, 'stream closed prematurely')
    }
  } else {
    loggerInstance.warn({ err }, 'response terminated with an error with headers already sent')
  }
}
