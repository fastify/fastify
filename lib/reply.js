...
// Caches the serialize functions compiled for a single schema, keyed by
// `${httpStatus}:${contentType}`. Keeps a direct reference to the
// first-compiled fn so callers can read it without knowing the cache is
// backed by a Map.
class SerializeFnCache {
  constructor () {
    this.fns = new Map()
...
Reply.prototype.getSerializationFunction = function (schemaOrStatus, contentType) {
  ...
  if (typeof schemaOrStatus === 'string' || typeof schemaOrStatus === 'number') {
    ...
    serialize = this[kRouteContext][kReplyCacheSerializeFns]?.get(schemaOrStatus)?.first
  }
...
Reply.prototype.compileSerializationSchema = function (schema, httpStatus = null, contentType = null) {
  ...
  const cacheKey = `${httpStatus}:${contentType}`

  // Check if serialize function already compiled
  let cachedFns = this[kRouteContext][kReplyCacheSerializeFns]?.get(schema)
  if (cachedFns != null && cachedFns.has(cacheKey)) {
    return cachedFns.get(cacheKey)
  }
  ...
  cachedFns = this[kRouteContext][kReplyCacheSerializeFns].get(schema)
  if (cachedFns == null) {
    cachedFns = new SerializeFnCache()
    this[kRouteContext][kReplyCacheSerializeFns].set(schema, cachedFns)
  }

  cachedFns.set(cacheKey, serializeFn)
