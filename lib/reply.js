...
function wrapOnSendEnd (err, reply, payload) {
  if (err) {
    err = annotateError(err, errorCodes.FST_ERR_ON_RESPONSE_HOOK)
...
function sendStream (payload, res, reply) {
  ...
  eos(payload, { readable: true, writable: false }, function (err) {
    sourceOpen = false
    if (err) {
      err = annotateError(err, errorCodes.FST_ERR_PAYLOAD_STREAM)
      if (res.headersSent) {
  ...
  eos(res, function (err) {
    if (err) {
      err = annotateError(err, errorCodes.FST_ERR_RESPONSE_STREAM)
      if (res.headersSent) {
        res.log.error(err, 'response terminated with an error with headers already sent')
      }
      if (sourceOpen) {
  ...
  if (customErrorHandler && reply._customError === false) {
    ...
    if (result && typeof result.then === 'function') {
      result
        ...
        .catch(err => reply.send(annotateError(err, errorCodes.FST_ERR_CUSTOM_ERR_HANDLER_REJECTION)))
    }
    return
  }
