...
function onSendEnd (reply, payload) {
  ...
  if ((statusCode >= 100 && statusCode < 200) || statusCode === 204 || statusCode === 205 || statusCode === 304) {
    ...
    if (statusCode === 205) {
      // Node treats only 1xx, 204 and 304 as bodyless, so a 205 without an
      // explicit length falls back to chunked encoding and still emits the
      // "0\r\n\r\n" terminator. RFC 9110 §8.6 forbids Content-Length only
      // for 1xx and 204, so declaring a zero length is valid here.
      reply[kReplyHeaders]['content-length'] = '0'
    }
    safeWriteHead(reply, statusCode)
