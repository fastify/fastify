...
Reply.prototype.send = function (payload) {
  if (this.sent) {
    this._req.logger.warn(new Error('Reply already sent'))
    return
  }
  ...
  if (payload && payload.isBoom) {
    this._req.logger.error(payload)
...
function handleError (reply, err, cb) {
  ...
  reply._req.logger.error({ res: reply.res, err }, err.message)
