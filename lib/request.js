...
function getRemoteAddress (rawRequest) {
  return rawRequest.socket?.remoteAddress ?? rawRequest.connection?.remoteAddress
}

function getProxyAddrRequest (rawRequest) {
  return {
    headers: rawRequest.headers,
    socket: {
      remoteAddress: getRemoteAddress(rawRequest)
    }
  }
}
...
function buildRequestWithTrustProxy (R, trustProxy) {
  ...
  Object.defineProperties(_Request.prototype, {
    ip: {
      get () {
        const addrs = proxyAddr.all(getProxyAddrRequest(this.raw), proxyFn)
        return addrs[addrs.length - 1]
      }
    },
    ips: {
      get () {
        return proxyAddr.all(getProxyAddrRequest(this.raw), proxyFn)
      }
    },
    host: {
      get () {
        if (this.headers['x-forwarded-host'] && proxyFn(getRemoteAddress(this.raw), 0)) {
    ...
    protocol: {
      get () {
        if (this.headers['x-forwarded-proto'] && proxyFn(getRemoteAddress(this.raw), 0)) {
...
Object.defineProperties(Request.prototype, {
  ...
  ip: {
    get () {
      const remoteAddress = getRemoteAddress(this.raw)
