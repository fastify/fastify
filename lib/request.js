...
// Caches the validate functions compiled for a single schema, keyed by
// `${httpPart}`. Keeps a direct reference to the first-compiled fn so
// metadata-less reads can return a function without knowing the cache is
// backed by a Map.
class ValidateFnCache {
  constructor () {
    this.fns = new Map()
    this.first = null
  }
  ...
  has (cacheKey) {
    return this.fns.has(cacheKey)
  }

  get (cacheKey) {
    return this.fns.get(cacheKey)
  }
  ...
  Object.defineProperties(_Request.prototype, {
    ...
    host: {
      get () {
        ...
        // The headers getter may allocate a new object, so read it once.
        const headers = this.headers
        if (headers['x-forwarded-host'] && socket != null && proxyFn(socket.remoteAddress, 0)) {
          return getLastEntryInMultiHeaderValue(headers['x-forwarded-host'])
        }
        /**
    ...
    protocol: {
      get () {
        ...
        if (headers['x-forwarded-proto'] && socket != null && proxyFn(socket.remoteAddress, 0)) {
          return getLastEntryInMultiHeaderValue(headers['x-forwarded-proto'])
        }
        if (this.socket) {
...
Object.defineProperties(Request.prototype, {
  ...
  getValidationFunction: {
    value: function (httpPartOrSchema) {
      if (typeof httpPartOrSchema === 'string') {
        ...
        return this[kRouteContext][kRequestCacheValidateFns]?.get(httpPartOrSchema)?.first
      }
    }
  },
  compileValidationSchema: {
    value: function (schema, httpPart = null) {
      ...
      const cacheKey = `${httpPart}`

      const cachedSchemaCache = this[kRouteContext][kRequestCacheValidateFns]?.get(schema)
      if (cachedSchemaCache?.has(cacheKey)) {
        return cachedSchemaCache.get(cacheKey)
      }
      ...
      let schemaCache = this[kRouteContext][kRequestCacheValidateFns].get(schema)
      if (schemaCache == null) {
        schemaCache = new ValidateFnCache()
        this[kRouteContext][kRequestCacheValidateFns].set(schema, schemaCache)
      }
      schemaCache.set(cacheKey, validateFn)
  ...
  validateInput: {
    value: function (input, schema, httpPart) {
      ...
      if (validate == null) {
        const schemaCache = this[kRouteContext][kRequestCacheValidateFns]?.get(schema)
        if (schemaCache != null) {
          // Metadata-less reads keep the first-compiled fn, matching the
          // serialization cache behavior; a part always gets its own fn
          validate = httpPart == null
            ? schemaCache.first
            : schemaCache.get(`${httpPart}`)
        }
        if (validate == null) {
          // We proceed to compile if there's no validate function yet
          validate = this.compileValidationSchema(schema, httpPart ?? null)
        }
      }
