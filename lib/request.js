...
function getTrustProxyAddresses (req, proxyFn) {
  const addrs = proxyAddr.all(req, proxyFn)
  return addrs[0] == null ? addrs.filter(addr => addr != null) : addrs
}

function hasTrustedProxyConnection (remoteAddress, proxyFn) {
  return remoteAddress != null && proxyFn(remoteAddress, 0)
}
...
function buildRequestWithTrustProxy (R, trustProxy) {
  ...
  Object.defineProperties(_Request.prototype, {
    ip: {
      get () {
        const addrs = getTrustProxyAddresses(this.raw, proxyFn)
    ...
    host: {
      get () {
        if (this.headers['x-forwarded-host'] && hasTrustedProxyConnection(this.raw.socket?.remoteAddress, proxyFn)) {
    ...
    protocol: {
      get () {
        if (this.headers['x-forwarded-proto'] && hasTrustedProxyConnection(this.raw.socket?.remoteAddress, proxyFn)) {
