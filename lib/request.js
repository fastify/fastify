...
Object.defineProperties(Request.prototype, {
  ...
  validate: {
    value: function (schema, input, httpPart) {
      ...
      if (validate == null) {
        // Check if validate function already compiled
        if (this.context[kRequestValidateFns].has(schema)) {
          ...
          if (validatorCompiler == null) {
            ...
            this.context.config.validatorCompiler = this.server[kSchemaController].validatorCompiler
          }

          validate = this.context.config.validatorCompiler({ schema, method, url, httpPart })
  ...
  serialize: {
    value: function (schema, input, httpStatus) {
      const { method, url } = this
      let serialize = null

      if (httpStatus != null) {
        serialize = this.context[kSchemaResponse] != null ? this.context[kSchemaResponse][httpStatus] : null
      }

      if (serialize == null) {
        // Check if serialize function already compiled
        if (this.context[kRequestSerializeFns].has(schema)) {
          serialize = this.context[kRequestSerializeFns].get(schema)
        } else {
          const { serializerCompiler } = this.context.config

          // If the SchemaController.serializer has not been set for the request
          // We prepare it here and use it right away
          if (serializerCompiler == null) {
            this.server[kSchemaController].setupSerializer(this.server[kOptions])
            this.context.config.serializerCompiler = this.server[kSchemaController].serializerCompiler
          }

          serialize = this.context.config.serializerCompiler({ schema, method, url, httpStatus })
          this.context[kRequestSerializeFns].set(schema, serialize)
        }
      }
