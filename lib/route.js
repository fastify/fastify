...
function buildRouting (options) {
  ...
  return {
    ...
    setup (options, fastifyArgs) {
      ...
      // Compute valid log levels once at setup time, not on every route registration
      if (hasLogger && logger.levels) {
        validLogLevels = [...Object.keys(logger.levels.values), 'silent']
      }
  ...
  function route ({ options, isFastify }) {
    ...
    function addNewRoute ({ path, prefixing = false, isFastify = false }) {
      ...
      // Validate logLevel if explicitly specified (not empty string which means 'use default')
      if (opts.logLevel && validLogLevels) {
        if (!validLogLevels.includes(opts.logLevel)) {
          throw new FST_ERR_ROUTE_INVALID_LOG_LEVEL(
            opts.logLevel,
            opts.method,
            opts.url,
            validLogLevels.join(', ')
