...
function buildRouting (options) {
  ...
  function route ({ options, isFastify }) {
    ...
    if (hasLogger && opts.logLevel !== undefined) {
      validateRouteLogLevelOption(opts.logLevel, opts.method, path, logger)
    }
...
function getValidLogLevels (logger) {
  const values = logger?.levels?.values
  if (values == null || typeof values !== 'object') return null

  const levels = Object.keys(values)
  if (!levels.includes('silent')) levels.push('silent')
  levels.sort()
  return levels
}

function validateRouteLogLevelOption (logLevel, method, path, logger) {
  const validLevels = getValidLogLevels(logger)

  if (typeof logLevel !== 'string') {
    throw new FST_ERR_ROUTE_LOG_LEVEL_INVALID(Object.prototype.toString.call(logLevel), method, path, validLevels?.join(', ') || 'unknown')
  }

  if (logLevel === 'silent') return

  if (validLevels == null) return

  if (!validLevels.includes(logLevel)) {
    throw new FST_ERR_ROUTE_LOG_LEVEL_INVALID(logLevel, method, path, validLevels.join(', '))
  }
}
