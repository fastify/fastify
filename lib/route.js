...
const {
  ...
  FST_ERR_LOG_INVALID_LOGGER_LEVEL,
...
function buildRouting (options) {
  ...
  function prepareRoute ({ method, url, options, handler, isFastify }) {
    ...
    if (options && typeof options.logLevel === 'string' &&
      ...
      !Object.keys(logger.levels.labels).some(k => logger.levels.labels[k] === options.logLevel)) {
      throw new FST_ERR_LOG_INVALID_LOGGER_LEVEL()
    }
