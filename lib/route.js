...
function buildRouterOptions (options, defaultOptions) {
  const hasRouterOptions = options.routerOptions !== undefined
  const routerOptions = hasRouterOptions ? { ...options.routerOptions } : {}

  for (const key of routerKeys) {
    // prefer top-level options, then routerOptions, then defaults
    if (options[key] !== undefined) {
      if (hasRouterOptions && routerOptions[key] !== undefined) {
        FSTDEP022()
      }
      routerOptions[key] = options[key]
    } else if (routerOptions[key] === undefined) {
      routerOptions[key] = defaultOptions[key]
    }
  }
