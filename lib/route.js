...
function buildRouting (options) {
  ...
  function hasRoute ({ options }) {
    return router.routes.some(route => route.method === options.method && route.path === options.url && equal(route.opts.constraints, options.constraints || {}))
  }
  ...
  function route ({ options, isFastify }) {
    ...
    function addNewRoute ({ path, prefixing = false, isFastify = false }) {
      ...
      const headHandler = router.routes.find(route => route.method === 'HEAD' && route.path === opts.url && equal(route.opts.constraints, constraints))
      const hasHEADHandler = headHandler !== undefined
