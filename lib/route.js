...
function buildRouting (options) {
  ...
  function route (options) {
    ...
    if (Array.isArray(opts.method)) {
      for (let i = 0, len = opts.method.length; i < len; ++i) {
    ...
    function afterRouteAdded ({ path, prefixing = false }, notHandledErr, done) {
      ...
      if (prefixing === false) {
      // run 'onRoute' hooks
        const hooks = this[kHooks].onRoute
        for (let i = 0, len = hooks.length; i !== len; ++i) {
          try {
            hooks[i].call(this, opts)
          } catch (error) {
            done(error)
            return
          }
      ...
      avvio.once('preReady', () => {
        for (let i = 0, len = lifecycleHooks.length; i !== len; ++i) {
          const hook = lifecycleHooks[i]
