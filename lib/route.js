...
const { FSTDEP007, FSTDEP008 } = require('./warnings')
...
function buildRouting(options) {
  ...
  function hasRoute({ options }) {
    return router.hasRoute(options.method, options.url || '', options.constraints)
  }
      ...
      this.after((notHandledErr, done) => {
        ...
        avvio.once('preReady', () => {
          ...
          if (opts.schema) {
            ...
            try {
              const isCustom =
                typeof opts?.validatorCompiler === 'function' || schemaController.isCustomValidatorCompiler
