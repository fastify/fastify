...
function buildRouting (options) {
  ...
  function route ({ options, isFastify }) {
    ...
    function addNewRoute ({ path, prefixing = false, isFastify = false }) {
      ...
      opts.requestLogLevel = opts.requestLogLevel || this[kRequestLogLevel]
      validateLogLevelOption(opts.logLevel, opts.method, opts.url, logger, 'logLevel')
      validateRequestLogLevelOption(opts.requestLogLevel, opts.method, opts.url, logger)
      ...
      const context = new Context({
        ...
        requestLogLevel: opts.requestLogLevel,
      ...
      this.after((notHandledErr, done) => {
        ...
        context.requestLogLevel = opts.requestLogLevel
...
function validateLogLevelOption (logLevel, method, path, logger, option) {
  ...
  if (typeof logLevel !== 'string' || logger.levels.values[logLevel] === undefined) {
    throw new FST_ERR_ROUTE_LOG_LEVEL_INVALID(option, method, path, logLevel)
  }
}

function validateRequestLogLevelOption (requestLogLevel, method, path, logger) {
  if (requestLogLevel === 'silent') return

  if (logger?.levels?.values != null) {
    validateLogLevelOption(requestLogLevel, method, path, logger, 'requestLogLevel')
    return
  }

  if (typeof requestLogLevel !== 'string' || typeof logger?.[requestLogLevel] !== 'function') {
    throw new FST_ERR_ROUTE_LOG_LEVEL_INVALID('requestLogLevel', method, path, requestLogLevel)
  }
}
