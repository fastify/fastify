...
function buildRouting (options) {
  ...
  function route (options) {
    ...
    // TODO: remove once review is done
    ...
    function addNewRoute (path, prefixing = false) {
      ...
      const headRouteExists = opts.method === 'HEAD' && router.find('HEAD', url, constraints) != null

      // TODO: improve validation for auto expose HEAD route before
      // adding a new route
      ...
      **/
      ...
      if (!headRouteExists) {
        try {
          ...
          const isDuplicatedRoute = Array.isArray(error.message.match(`Method '${opts.method}' already declared for route '${opts.url}'`))
        ...
        avvio.once('preReady', () => {
          ...
          if (opts.schema) {
            ...
            try {
              ...
              done(new FST_ERR_SCH_VALIDATION_BUILD(opts.method, url, error.message))
            }
            ...
            try {
              ...
              done(new FST_ERR_SCH_SERIALIZATION_BUILD(opts.method, url, error.message))
            }
          }
        })
