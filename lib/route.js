...
function buildRouting (options) {
  ...
  function route (options) {
    ...
    function addNewRoute (path, prefixing = false) {
      ...
      try {
        ...
        if (isDuplicatedRoute) {
          throw new FST_ERR_DUPLICATED_ROUTE(opts.method, opts.url, constraints)
        }
      ...
      this.after((notHandledErr, done) => {
        ...
        // Run hooks and more
        afterRouteAdded.call(this, path, url, context, notHandledErr, done)
      })
    }
     ...
     * @param {*} done callback
     */
    function afterRouteAdded (path, url, context, notHandledErr, done) {
      // TODO: remove once review is done
      // opts.url = url
      // opts.path = url
      // opts.routePath = path
