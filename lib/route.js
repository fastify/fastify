...
function buildRouting (options) {
  ...
  function route ({ options, isFastify }) {
    ...
    function addNewRoute ({ path, prefixing = false, isFastify = false }) {
      ...
      let hasHEADHandler = false

      if (shouldExposeHead && opts.method === 'GET') {
        // We're using the "router.on" method in order to understand if there is an identical route
        // in router, only with "HEAD" method.
        // once "has" method will be implemented on 'find-my-way' side, we will use it.

        let headHandler = null

        try {
          router.on('HEAD', opts.url, { constraints }, routeHandler, context)
          router.off('HEAD', opts.url, constraints)
        } catch (error) {
          const isDuplicatedRoute = error.message.includes("Method 'HEAD' already declared for route")
          if (isDuplicatedRoute) {
            hasHEADHandler = true
            headHandler = router.find('HEAD', opts.url, constraints)
          }
        }

        // remove the head route created by fastify
        if (hasHEADHandler && !context[kRouteByFastify] && headHandler.store[kRouteByFastify]) {
          router.off('HEAD', headHandler.store.config.url, constraints)
        }
      }
