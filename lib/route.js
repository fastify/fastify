...
const {
  ...
  FST_ERR_ROUTE_INVALID_LOG_LEVEL_OPTION,
...
function buildRouting (options) {
  ...
  function route ({ options, isFastify }) {
    ...
    function addNewRoute ({ path, prefixing = false, isFastify = false }) {
      ...
      opts.routePath = path
      opts.prefix = prefix
      validateLogLevelOption(opts.logLevel)
...
function validateLogLevelOption (logLevel) {
  if (logLevel === undefined) return
  if (typeof logLevel !== 'string' || !validLogLevels.has(logLevel)) {
    throw new FST_ERR_ROUTE_INVALID_LOG_LEVEL_OPTION(logLevel)
  }
}
