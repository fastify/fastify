  ...
  function route (options) {
    ...
    if (Array.isArray(options.method)) {
      ...
      for (var i = 0; i < options.method.length; i++) {
        if (supportedMethods.indexOf(options.method[i]) === -1) {
          throw new Error(`${options.method[i]} method is not supported!`)
        }
      }
    } else {
      if (supportedMethods.indexOf(options.method) === -1) {
        throw new Error(`${options.method} method is not supported!`)
      }
    }

    if (!options.handler) {
      throw new Error(`Missing handler function for ${options.method}:${options.url} route.`)
    }

    if (options.errorHandler !== undefined && typeof options.errorHandler !== 'function') {
      throw new Error(`Error Handler for ${options.method}:${options.url} route, if defined, must be a function`)
    }
    ...
    this.after((notHandledErr, done) => {
      ...
      if (path === '/' && prefix.length > 0) {
        switch (options.prefixTrailingSlash) {
          case 'slash':
