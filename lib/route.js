...
const { defaultInitOptions } = require('./initial-config-validation.js')
...
function validateSchemaBodyOption (method, path, schema) {
  ...
  // Feature #5215: Align route params with schema at startup
  if (this[kOptions]?.alignRouteParams === true && path.includes(':')) {
    ...
    if (pathParams.length > 0) {
      const schemaParams = schema?.params?.properties ? Object.keys(schema.params.properties) : []
      
      for (const param of pathParams) {
        if (!schemaParams.includes(param)) {
          throw new Error(`Route param mismatch: '${param}' found in path '${path}' but is missing from the validation schema params.`)
        }
      }
    }
  }
...
function buildRouterOptions (options, defaultOptions) {
  ...
  for (const key of routerKeys) {
    if (!Object.hasOwn(routerOptions, key)) {
      // defaultInitOptions are non-function default
      // defaultOptions are function default
      routerOptions[key] = options[key] ?? defaultInitOptions.routerOptions[key] ?? defaultOptions[key]
    }
  }
