...
function buildRouting (options) {
  ...
  function route ({ options, isFastify }) {
    ...
    function addNewRoute ({ path, prefixing = false, isFastify = false }) {
      ...
      opts.routePath = path
      opts.prefix = prefix
      // Only validate logLevel against pino default levels when using the default logger.
      // Custom loggers may support non-standard levels; skip validation when loggerInstance is set.
      if (opts.logLevel !== undefined) {
        const hasCustomLogger = this[kOptions] && this[kOptions].loggerInstance
        if (!hasCustomLogger) {
          const validLevels = { trace: 1, debug: 1, info: 1, warn: 1, error: 1, fatal: 1 }
          if (!validLevels[opts.logLevel]) {
            throw FST_ERR_LOG_INVALID_LEVEL(opts.logLevel)
          }
        }
      }
