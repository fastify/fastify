...
function buildRouting (options) {
  ...
  function route (opts) {
    ...
    function afterRouteAdded (path, notHandledErr, done) {
      ...
      if (opts.schema) {
        // TODO refactor all these try/catch
        try {
          context.schema = this[kSchemas].normalizeSchema(context.schema)
        } catch (error) {
          done(error)
          return
        }
        ...
        if (!opts.validatorCompiler && !this[kValidatorCompiler]) {
          ...
          this.setValidatorCompiler(buildSchemaValidatorCompiler(this[kSchemas], this[kOptions].ajv))
        }
        try {
          compileSchemasForValidation(context, opts.validatorCompiler || this[kValidatorCompiler], this[kSchemas])
        } catch (error) {
          // bubble up the FastifyError instance
          done(error.code ? error : new FST_ERR_SCH_VALIDATION_BUILD(opts.method, url, error.message))
          return
        }

        if (!opts.serializerCompiler && !this[kSerializerCompiler]) {
          // if the instance doesn't have a serializer, build the default one for the single fastify instance
          this.setSerializerCompiler(buildSchemaSerializerCompiler(this[kSchemas]))
        }
        try {
          compileSchemasForSerialization(context, opts.serializerCompiler || this[kSerializerCompiler], this[kSchemas])
        } catch (error) {
          // bubble up the FastifyError instance
          done(error.code ? error : new FST_ERR_SCH_SERIALIZATION_BUILD(opts.method, url, error.message))
          return
        }
      }
