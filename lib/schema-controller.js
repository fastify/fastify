...
function buildSchemaController (parentSchemaCtrl) {
  ...
  if (parentSchemaCtrl) {
    const oldSchemas = parentSchemaCtrl.getSchemas()
    for (const schemaId in oldSchemas) {
      // TODO add without clone
      contextSchemaCtrl.add(oldSchemas[schemaId])
    }
    contextSchemaCtrl.schemaBucket.newSchemasAdded = false // TODO remove it
