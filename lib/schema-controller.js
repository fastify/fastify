...
/**
 * This method is called once on the fastify server inizialization.
 * You may use it to set some globals argument as caching
 */
 ...
 * This should return a new SchemaController.
 * WARN: If it returns `this`, the JSON schema encapsulation will be broken
 */
SchemaController.prototype.clone = function clone () {
  const wrapper = new SchemaController()

  const clonedSchema = new Schemas()
  const oldSchemas = this.schemaBucket.getSchemas()
  for (const schemaId in oldSchemas) {
    clonedSchema.add(oldSchemas[schemaId])
  }
