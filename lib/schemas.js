...
Schemas.prototype.traverse = function (schema, refResolver) {
  for (var key in schema) {
    ...
    if (typeof schema[key] === 'string' && key !== '$schema' && key !== '$ref' && schema[key].slice(-1) === '#') {
      if (refResolver) {
        schema[key] = refResolver(schema[key].slice(0, -1))
      } else {
...
function getSchemaMap (schema, refResolver, composer = {}) {
  for (var key in schema) {
    if (key === '$ref') {
      const refValue = schema[key]
      const refId = refValue.slice(0, refValue.indexOf('#'))
      if (refId.length > 0) {
        composer[refId] = refResolver(refId)
        getSchemaMap(composer[refValue], refResolver, composer)
      }
    } else if (schema[key] !== null && typeof schema[key] === 'object') {
      getSchemaMap(schema[key], refResolver, composer)
    }
  }
  return composer
}

module.exports = { Schemas, buildSchemas, getSchemaMap }
