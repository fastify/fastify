...
function normalizeSchema (routeSchemas) {
  ...
  const keys = ['headers', 'querystring', 'params', 'body']
  ...
  if (routeSchemas.response) {
    ...
    for (let i = 0, len = httpCodes.length; i !== len; ++i) {
...
function generateFluentSchema (schema) {
  ...
  for (let i = 0, len = SCHEMAS_SOURCE.length; i !== len; ++i) {
    const key = SCHEMAS_SOURCE[i]
    const keyValue = schema[key]
    if (keyValue && (keyValue.isFluentSchema || keyValue[kFluentSchema])) {
      schema[key] = keyValue.valueOf()
    }
  }
  ...
  if (schema.response) {
    ...
    for (let i = 0, len = httpCodes.length; i !== len; ++i) {
      const code = httpCodes[i]
      const codeValue = schema.response[code]
      if (codeValue.isFluentSchema || codeValue[kFluentSchema]) {
        schema.response[code] = codeValue.valueOf()
      }
    }
  }
}
