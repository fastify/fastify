...
function normalizeSchema (routeSchemas, serverOptions) {
  ...
  if (routeSchemas.response) {
    ...
    for (const code of httpCodes) {
      ...
      // ducktyping to check if the media content is meant to be used or it's the short form.
      let mediaContent = false
      if (contentProperty) {
        const firstEntry = contentProperty[Object.keys(contentProperty)[0]]
        mediaContent = !!(typeof firstEntry === 'object' && firstEntry.schema)
      }

      if (mediaContent) {
        for (const mediaName in contentProperty) {
...
function getSchemaSerializer (context, statusCode, contentType) {
  ...
  if (responseSchemaDef[fallbackStatusCode]) {
    if (responseSchemaDef[fallbackStatusCode].constructor === Object) {
      ...
      if (responseSchemaDef[fallbackStatusCode][type]) {
        return responseSchemaDef[fallbackStatusCode][type]
      }
  ...
  if (responseSchemaDef.default) {
    if (responseSchemaDef.default.constructor === Object) {
      const type = contentType.split(';')[0]
      if (responseSchemaDef.default[type]) {
        return responseSchemaDef.default[type]
      }
