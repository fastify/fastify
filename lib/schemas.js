...
function normalizeSchema (routeSchemas, serverOptions) {
  ...
  if (routeSchemas.response) {
    ...
    for (const code of httpCodes) {
      if (Array.isArray(routeSchemas.response[code].contentTypes)) {
        for (const cureentResponse of routeSchemas.response[code].contentTypes) {
          cureentResponse.schema = getSchemaAnyway(cureentResponse.schema, serverOptions.jsonShorthand)
        }
      } else {
...
function getSchemaSerializer (context, statusCode, contentType) {
  ...
  if (responseSchemaDef[statusCode]) {
    if (responseSchemaDef[statusCode].constructor === Object) {
      ...
      if (responseSchemaDef[statusCode][type]) {
        return responseSchemaDef[statusCode][type]
      }
