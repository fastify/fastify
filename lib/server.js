...
const { kState, kOptions, kServerBindings, kHooks, kAvvioBoot } = require('./symbols')
    ...
    if (host === 'localhost') {
      listenOptions.cb = (err, address) => {
        ...
        multipleBindings.call(this, server, httpHandler, options, listenOptions, () => {
          ...
          // need to change this later, if we get hookRunnerApplication() to complete for cb===undefined
      ...
      if (host === 'localhost') {
        return listening.then(address => {
          return new Promise((resolve, reject) => {
            const rejectReady = reject
            // let resolveReady = resolve
            multipleBindings.call(this, server, httpHandler, options, listenOptions, () => {
              this[kState].listening = true
              // console.log('Listening true...call the hookRunner')
              // calling hookRunnerApplication for onListen, may be in the right direction, will result in infinite loop
              this[kAvvioBoot](() => {})
              hookRunnerApplication('onListen', this[kAvvioBoot], this, manageErr)
              // for (const hook of this[kHooks].onListen) {
              //   hook.call(this)
              // }
              function manageErr (err) {
                ...
                // as cause
                // console.log('manageErr error', err)
