...
function createServer (options, httpHandler) {
  ...
  function listen (listenOptions, ...args) {
    ...
    const serverFactoryListening = server.listening && options.serverFactory
    /* istanbul ignore else */
    if (serverFactoryListening) {
      throw new FST_ERR_DUPLICATE_SERVER()
    }
    if (host === 'localhost') {
  ...
  dns.lookup(listenOptions.host, { all: true }, (dnsErr, addresses) => {
    ...
    for (const adr of addresses) {
      if (adr.address !== primaryAddress.address) {
        ...
        /* istanbul ignore else */
        if (secondaryServer === mainServer) {
          warning.emit('FSTWARN001')
          onListen()
          continue
        }
