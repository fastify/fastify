...
function addAbortSignalOption (onAborted, server, listenOptions) {
  const onAbortedCallback = () => {
    onAborted()
  }

  if (listenOptions.signal) {
    listenOptions.signal.addEventListener('abort', onAbortedCallback)
    server.once('close', () => listenOptions.signal.removeEventListener('abort', onAbortedCallback))
  }
}
...
function createServer (options, httpHandler) {
  ...
  function listen (listenOptions, ...args) {
    ...
    addAbortSignalOption(this.close, server, listenOptions)
