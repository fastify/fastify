...
const { kState, kOptions, kServerBindings, kHooks } = require('./symbols')
const {
...
function createServer (options, httpHandler) {
  ...
  function listen (listenOptions, ...args) {
    console.log('start listen function') // start listen() comment
    ...
    if (arguments.length === 0) {
      ...
      console.log('argument length 3 else') // comment
      listenOptions.cb = cb

      console.log('listenOptions', listenOptions) //  comment
    }
    console.log('after argument lengths') // comment
    if (listenOptions.signal) {
      console.log('listenOptions.signal') // comment
    ...
    console.log('after signal') // comment
    ...
    if (host === 'localhost') {
      listenOptions.cb = (err, address) => {
        ...
        multipleBindings.call(this, server, httpHandler, options, listenOptions, () => {
          ...
          for (const hook of this[kHooks].onListen) {
            hook.call(this)
          }
    ...
    console.log('after host') // comment
    ...
    if (cb === undefined) {
      console.log('start of cb===undefined') // comment
      const listening = listenPromise.call(this, server, listenOptions)
      console.log('after listenPromise.call') // comment
      /* istanbul ignore else */
      if (host === 'localhost') {
        return listening.then(address => {
          console.log('after localhost then', listening) // comment
          return new Promise((resolve, reject) => {
            multipleBindings.call(this, server, httpHandler, options, listenOptions, () => {
              ...
              // this.hookRunnerApplication('onListen',this[kAvvioBoot],fastify,manageErr)
              // this.onListenHook()
              resolve(address)
            })
          })
      ...
      console.log('before return listening') // comment
      return listening
    }
  ...
  return this.ready().then(() => {
    ...
    const listen = new Promise((resolve, reject) => {
      server.listen(listenOptions, () => {
        console.log('in server.listen() in promise') //   comment
        server.removeListener('error', errEventHandler)
        resolve(logServerAddress.call(this, server, listenOptions.listenTextResolver || defaultResolveServerListeningText))

        console.log('end of server.listen() in promise') //   comment
      })
      // we set it afterwards because listen can throw
      this[kState].listening = true
      console.log('after server.listen() in promise') //  comment
    })
