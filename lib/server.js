...
const { FST_ERR_HTTP2_INVALID_VERSION, FST_ERR_REOPENED_CLOSE_SERVER, FST_ERR_REOPENED_SERVER, FST_ERR_HTTP_MAX_REQUESTS_PER_SOCKET_INVALID_VERSION } = require('./errors')
...
function createServer (options, httpHandler) {
  ...
  if (options.serverFactory) {
    ...
    server.keepAliveTimeout = options.keepAliveTimeout
    if (semver.lt(process.versions.node, '16.10.0') && options.maxRequestsPerSocket > 0) {
      // when node version smaller than 16.10.0 and is a number
      // it is not a valid option
      throw new FST_ERR_HTTP_MAX_REQUESTS_PER_SOCKET_INVALID_VERSION()
    } else if (options.maxRequestsPerSocket > 0) {
