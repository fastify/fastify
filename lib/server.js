...
function createServer (options, httpHandler) {
  const { server, validate } = getServerInstance(options, httpHandler)

  return { server, validate, listen }
  ...
  dns.lookup(listenOptions.host, { all: true }, (dnsErr, addresses) => {
    ...
    for (const adr of addresses) {
      if (adr.address !== primaryAddress.address) {
        ...
        const { server: secondaryServer } = getServerInstance(serverOpts, httpHandler)
...
// This function used to compile a validation function for http version
// bypass here is used to bypass the version check
function compileValidateHTTPVersion (allowed, bypass) {
  const obj = {}
  for (const code of allowed) {
    obj[code] = true
  }
  return function validateHTTPVersion (httpVersion) {
    return bypass || Object.prototype.hasOwnProperty.call(obj, httpVersion)
  }
}
...
function getServerInstance (options, httpHandler) {
  ...
  if (options.serverFactory) {
    ...
    validate = compileValidateHTTPVersion([], true)
  } else if (options.http2) {
    if (options.https) {
      server = http2().createSecureServer(options.https, httpHandler)
      validate = compileValidateHTTPVersion(options.https.allowHTTP1 === true ? ['2.0', '1.1', '1.0'] : ['2.0'], false)
    } else {
      server = http2().createServer(httpHandler)
      validate = compileValidateHTTPVersion(['2.0'], false)
    }
    ...
    validate = compileValidateHTTPVersion(['1.1', '1.0'], false)
  }
