    ...
    if (host === 'localhost') {
      listenOptions.cb = (err, address) => {
        ...
        multipleBindings.call(this, server, httpHandler, options, listenOptions, () => {
          ...
          // change this later, if we complete hookRunnerApplication() for cb===undefined
      ...
      if (host === 'localhost') {
        return listening.then(address => {
          return new Promise((resolve, reject) => {
            const rejectListen = reject
            // const resolveListen = resolve, resolve doesn't work yet, so not implemented
            multipleBindings.call(this, server, httpHandler, options, listenOptions, () => {
              this[kState].listening = true
              this[kAvvioBoot](() => {})
              ...
              function manageErr (err) {
                if (err) {
                  return rejectListen(err)
                }
                // resolveListen(this) currently implementing resolve causes listen and deprecated listen tests to fail due to address===null
              }
              resolve(address)
            })
          })
