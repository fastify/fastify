...
function createServer (options, httpHandler) {
  ...
  function listen (
    ...
    if (listenOptions.signal) {
      ...
      this[kState].abortSignal = signal

      if (signal.aborted) {
        this[kState].abortedReason = signal.reason
        this.close()
      } else {
        const onAborted = () => {
          this[kState].abortedReason = signal.reason
...
function multipleBindings (mainServer, httpHandler, serverOpts, listenOptions, onListen) {
  ...
  dns.lookup(listenOptions.host, { all: true }, (dnsErr, addresses) => {
    if (dnsErr || this[kState].abortedReason) {
      // not blocking the main server listening
      // this.log.warn('dns.lookup error:', dnsErr)
      // or while invoking dns.lookup, listening was aborted
      onListen()
      return
    }
...
function listenCallback (server, listenOptions) {
  ...
  return (err) => {
    if (signal?.aborted) {
      // Prevent `listenOptions.cb(err)` from being calling twice. In all cases there is `listenOptions.cb(err, null)` inside `wrap`:
      //  1. `err` can be from `signal.addEventListener('abort', doRejectPromise, { once: true })`.
      //  2. `err` can be from `this[kState].readyPromise` in fastify.js~ready.
      return
    }
    ...
    if (!this[kState].closing) {
      ...
      this[kState].listening = true
    } else if (this[kState].abortedReason) {
      wrap(this[kState].abortedReason)
    }
  }
}
