  ...
  return function _routeHanlder (params, req, res) {
    ...
    if (req.method === 'OPTIONS') {
      if (req.headers['content-type']) {
        ...
        } else if (customParsing.checkHeader(req.headers['content-type'])) {
          return customParsing.run(req.headers['content-type'], handler, hooks, handle, params, req, res, urlUtil.parse(req.url, true).query)
        }
        res.statusCode = 415
        setImmediate(wrapResEnd, res)
        return
...
function build (url, router, hooks, customParsing) {
  const node = {}
  router.on(url, routerHandler(node, hooks, customParsing))
