...
/**
 * This function is called whenever an instance of `Error`
 * must be serialized
 */
function serializeError (context, data, statusCode) {
  const responseSchemaDef = context[responseSchema]
  if (!responseSchemaDef) {
    return fastSerializeError(data)
  }
  if (responseSchemaDef[statusCode]) {
    return responseSchemaDef[statusCode](data)
  }
