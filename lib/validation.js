...
function validate (context, request, callback) {
  validateParam(context[paramsSchema], request, 'params', function (params) {
    if (params) {
      return callback(wrapValidationError(params, 'params', context.schemaErrorFormatter))
    }

    validateParam(context[bodySchema], request, 'body', function (body) {
      if (body) {
        return callback(wrapValidationError(body, 'body', context.schemaErrorFormatter))
      }

      validateParam(context[querystringSchema], request, 'query', function (query) {
        if (query) {
          return callback(wrapValidationError(query, 'querystring', context.schemaErrorFormatter))
        }

        validateParam(context[headersSchema], request, 'headers', function (headers) {
          if (headers) {
            return callback(wrapValidationError(headers, 'headers', context.schemaErrorFormatter))
          }
