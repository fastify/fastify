...
const { getSchemaMap } = require('./schemas')
...
function getValidatorForStatusCodeSchema (statusCodeDefinition, externalSchema, schemaResolver) {
  if (schemaResolver) {
    const resolvedSchema = getSchemaMap(statusCodeDefinition, schemaResolver)
    return fastJsonStringify(statusCodeDefinition, { schema: resolvedSchema })
  }
  return fastJsonStringify(statusCodeDefinition, { schema: externalSchema })
}

function getResponseSchema (responseSchemaDefinition, sharedSchemas, schemaResolver) {
  var statusCodes = Object.keys(responseSchemaDefinition)
  return statusCodes.reduce(function (r, statusCode) {
    r[statusCode] = getValidatorForStatusCodeSchema(responseSchemaDefinition[statusCode], sharedSchemas, schemaResolver)
    return r
  }, {})
}
...
function build (context, compile, schemas, schemaResolver) {
  ...
  context.schema = schemas.resolveRefs(context.schema, schemaResolver)
  ...
  if (context.schema.response) {
    context[responseSchema] = getResponseSchema(context.schema.response, schemas.getSchemas(), schemaResolver)
  }
