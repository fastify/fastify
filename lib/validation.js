...
function compileSchemasForValidation (context, compile, isCustom, warnings) {
  ...
  if (headers && (isCustom || Object.getPrototypeOf(headers) !== Object.prototype)) {
    ...
    warnings.emit('FSTWRN001', 'headers', method, url)
  }
  ...
  if (schema.body) {
    ...
    warnings.emit('FSTWRN001', 'body', method, url)
  }

  if (schema.querystring) {
    context[querystringSchema] = compile({ schema: schema.querystring, method, url, httpPart: 'querystring' })
  } else if (Object.hasOwn(schema, 'querystring')) {
    warnings.emit('FSTWRN001', 'querystring', method, url)
  }

  if (schema.params) {
    context[paramsSchema] = compile({ schema: schema.params, method, url, httpPart: 'params' })
  } else if (Object.hasOwn(schema, 'params')) {
    warnings.emit('FSTWRN001', 'params', method, url)
  }
}
