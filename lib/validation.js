...
const { ValidatorCompiler: DefaultValidatorCompiler } = require('./schema-compiler')
...
function compileSchemasForValidation (context, compile, schemas) {
  ...
  if (headers && Object.getPrototypeOf(headers) !== Object.prototype) {
    ...
    context[headersSchema] = compile('headers', headers)
  } else if (headers) {
    ...
    context[headersSchema] = compile('headers', headersSchemaLowerCase)
  }

  if (context.schema.body) {
    context[bodySchema] = compile('body', context.schema.body)
  }

  if (context.schema.querystring) {
    context[querystringSchema] = compile('querystring', context.schema.querystring)
  }

  if (context.schema.params) {
    context[paramsSchema] = compile('params', context.schema.params)
  }
}
