...
const {
  ValidatorCompiler: DefaultValidatorCompiler
...
function compileSchemasForSerialization (context, compile, schemas) {
  if (!context.schema) {
    return
  }
...
function compileSchemasForValidation (context, compile, schemas) {
  if (!context.schema) {
    return
  }

  generateFluentSchema(context.schema)
  context.schema = schemas.normalizeSchema(context.schema)
  ...
  if (headers && Object.getPrototypeOf(headers) !== Object.prototype) {
    ...
    context[headersSchema] = compile(context.config.url, 'headers', headers)
  } else if (headers) {
    ...
    context[headersSchema] = compile(context.config.url, 'headers', headersSchemaLowerCase)
  }

  if (context.schema.body) {
    context[bodySchema] = compile(context.config.url, 'body', context.schema.body)
  }

  if (context.schema.querystring) {
    context[querystringSchema] = compile(context.config.url, 'querystring', context.schema.querystring)
  }

  if (context.schema.params) {
    context[paramsSchema] = compile(context.config.url, 'params', context.schema.params)
  }
}
...
function buildSchemaValidatorCompiler (schemaBuckets, options) {
  return DefaultValidatorCompiler(schemaBuckets.getSchemas(), options, schemaBuckets.cache)
}

function buildSchemaSerializerCompiler (schemaBuckets, options) {
  return DefaultValidatorCompiler(schemaBuckets.getSchemas(), options, schemaBuckets.cache)
}
