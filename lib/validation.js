...
function stripSchemaKeyword (schema) {
  if (schema && typeof schema === 'object' && schema.$schema) {
    const { $schema, ...schemaWithoutKeyword } = schema
    return schemaWithoutKeyword
  }
  return schema
}
...
function compileSchemasForValidation (context, compile, isCustom) {
  ...
  if (headers && (isCustom || Object.getPrototypeOf(headers) !== Object.prototype)) {
    ...
    context[headersSchema] = compile({ schema: stripSchemaKeyword(headers), method, url, httpPart: 'headers' })
  } else if (headers) {
    ...
    context[headersSchema] = compile({ schema: stripSchemaKeyword(headersSchemaLowerCase), method, url, httpPart: 'headers' })
  ...
  if (schema.body) {
    ...
    if (contentProperty) {
      ...
      for (const contentType of Object.keys(contentProperty)) {
        ...
        contentTypeSchemas[contentType] = compile({ schema: stripSchemaKeyword(contentSchema), method, url, httpPart: 'body', contentType })
      }
      context[bodySchema] = contentTypeSchemas
    } else {
      context[bodySchema] = compile({ schema: stripSchemaKeyword(schema.body), method, url, httpPart: 'body' })
    }
  ...
  if (schema.querystring) {
    context[querystringSchema] = compile({ schema: stripSchemaKeyword(schema.querystring), method, url, httpPart: 'querystring' })
  } else if (Object.hasOwn(schema, 'querystring')) {
    FSTWRN001('querystring', method, url)
  }

  if (schema.params) {
    context[paramsSchema] = compile({ schema: stripSchemaKeyword(schema.params), method, url, httpPart: 'params' })
