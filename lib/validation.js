...
function build (context, compile, schemas) {
  ...
  const querystring = context.schema.querystring || context.schema.query
  ...
  if (querystring) {
    context[querystringSchema] = compile(querystring)
  }
...
function validate (context, request) {
  ...
  if (query) {
    const dataVar = context.schema.querystring ? 'querystring' : 'query'
    return wrapValidationError(query, dataVar)
  }
