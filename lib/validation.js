...
function getOutputSchema (responseSchemaDefinition) {
  var schemas = { }
  var statusCodes = Object.keys(responseSchemaDefinition)
  for (var i = 0; i < statusCodes.length; i++) {
    var statusCode = statusCodes[i]
    schemas[statusCode] = getValidatorForStatusCodeSchema(responseSchemaDefinition[statusCode])
  }
  return schemas
}
...
function build (opts) {
  ...
  if (opts.schema.response) {
    opts[responseSchema] = getOutputSchema(opts.schema.response)
  }
