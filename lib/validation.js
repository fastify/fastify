...
function compileSchemasForValidation (context, compile, isCustom) {
  ...
  if (schema.body) {
    ...
    if (contentProperty) {
      ...
      for (const mediaName of Object.keys(contentProperty)) {
        const contentSchema = contentProperty[mediaName].schema
...
function validate (context, request, execution) {
  ...
  if (runExecution || !execution.skipBody) {
    ...
    if (typeof context[bodySchema] === 'function') {
      ...
      const mediaName = request.headers['content-type']?.split(';', 1)[0]
      const contentSchema = context[bodySchema][mediaName]
      if (contentSchema) {
