...
function build (opts) {
  if (!opts.schema) {
    opts[responseSchema] = {default: fastSafeStringify}
    return
  }
...
function serializeAccordingToResponseSchema (responseSchema, data, statusCode) {
  ...
  return responseSchema.default(data)
}

function serialize (handle, data, statusCode) {
  return serializeAccordingToResponseSchema(handle[responseSchema], data, statusCode)
}
