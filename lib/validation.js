...
function lazySlot (context) {
  if (context[lazySchema] === undefined) {
    context[lazySchema] = { validation: null, serialization: null, error: null }
  }
...
function runLazyBuild (context, key, ErrorClass) {
  ...
  if (lazy.error !== null) throw lazy.error
  if (lazy[key] === null) return
  ...
  try {
    ...
    lazy.validation = null
    lazy.serialization = null
    const { method, url } = context.config
