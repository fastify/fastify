...
function addOutputSchemaToOptions (opts, responseSchemaDefinition) {
  opts[responseSchema] = { }
  var statusCodes = Object.keys(responseSchemaDefinition)
  for (var i = 0; i < statusCodes.length; i++) {
    var statusCode = statusCodes[i]
    opts[responseSchema][statusCode] = getValidatorForStatusCodeSchema(responseSchemaDefinition[statusCode])
  }
}
...
function build (opts) {
  ...
  if (opts.schema.response) {
    addOutputSchemaToOptions(opts, opts.schema.response)
  }
