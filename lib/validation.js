...
function validate (context, request) {
  ...
  var schemaErrorFormatter = (context.schemaErrorFormatter || schemaErrorsText).bind(request)

  if (params) {
    return wrapValidationError(params, 'params', schemaErrorFormatter)
  }
  var body = validateParam(context[bodySchema], request, 'body')
  if (body) {
    return wrapValidationError(body, 'body', schemaErrorFormatter)
  }
  var query = validateParam(context[querystringSchema], request, 'query')
  if (query) {
    return wrapValidationError(query, 'querystring', schemaErrorFormatter)
  }
  var headers = validateParam(context[headersSchema], request, 'headers')
  if (headers) {
    return wrapValidationError(headers, 'headers', schemaErrorFormatter)
  }
  return null
}
...
function wrapValidationError (result, dataVar, schemaErrorFormatter) {
  ...
  try {
    validationError.message = schemaErrorFormatter(result, dataVar)
  } catch (err) {
    validationError.formatterError = `schema custom formatter error: ${err.message}`
