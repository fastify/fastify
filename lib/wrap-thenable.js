...
function wrapThenable (thenable, reply, store) {
  ...
  thenable.then(function (payload) {
    ...
    try {
      // an onError hook is still running for an error sent with reply.send():
      // the error response is already on its way, so there is nothing to send
      // (calling reply.send() here would throw FST_ERR_SEND_INSIDE_ONERR)
      if (reply[kReplyIsRunningOnErrorHook] === true) {
        if (payload !== undefined) {
          reply.log.warn('Promise resolved with a payload, but an error response is already being sent')
        }
        return
      }
    ...
    try {
      ...
      if (reply[kReplyIsRunningOnErrorHook] === true) {
        reply.log.error({ err }, 'Promise errored, but an error response is already being sent')
        return
      }
