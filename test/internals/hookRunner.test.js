...
test('onSendHookRunner - Basic', t => {
  ...
  function fn1 (request, reply, payload, done) {
    t.assert.deepEqual(request, originalRequest)
    t.assert.deepEqual(reply, originalReply)
...
test('onSendHookRunner - Can change the payload', t => {
  ...
  function fn1 (request, reply, payload, done) {
    t.assert.deepEqual(payload, v1)
    done(null, v2)
  }

  function fn2 (request, reply, payload, done) {
    t.assert.deepEqual(payload, v2)
    done(null, v3)
  }

  function fn3 (request, reply, payload, done) {
    t.assert.deepEqual(payload, v3)
    done(null, v4)
  }
