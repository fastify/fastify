...
test('#compileValidationSchema', async (subtest) => {
  ...
  await subtest.test('Should reuse the validate fn across multiple invocations - Route without schema', async (t) => {
    const fastify = Fastify()
    let validate = null
    let counter = 0
  ...
  await subtest.test('Should use the custom validator compiler for the route', async (t) => {
    const fastify = Fastify()
    let called = 0
  ...
  await subtest.test('Should instantiate a WeakMap when executed for first time', async (t) => {
    const fastify = Fastify()
...
test('#getValidationFunction', async (subtest) => {
  ...
  await subtest.test('Should return the validation function from each HTTP part', async (t) => {
    const fastify = Fastify()
      ...
      (req, reply) => {
        ...
        switch (params.id) {
          ...
          case 4:
            ...
            break
          case 5:
            t.assert.strictEqual(customValidation, req.getValidationFunction(defaultSchema))
...
test('#validate', async (subtest) => {
  ...
  await subtest.test('Should return true/false if input valid - Route without schema', async (t) => {
    const fastify = Fastify()
  ...
  await subtest.test('Should use the custom validator compiler for the route', async (t) => {
    ...
    fastify.get('/', { validatorCompiler: custom }, (req, reply) => {
      const ok = req.validateInput({ hello: 'world' }, defaultSchema, 'querystring')
  ...
  await subtest.test('Should return true/false if input valid - With Schema for Route defined', async (t) => {
    const fastify = Fastify()
  ...
  await subtest.test('Should throw if missing validation fn for HTTP part and not schema provided', async (t) => {
    const fastify = Fastify()
  ...
  await subtest.test('Should throw if missing validation fn for HTTP part and not valid schema provided', async (t) => {
    const fastify = Fastify()
  ...
  await subtest.test('Should set a WeakMap if compiling the very first schema', async (t) => {
    const fastify = Fastify()
...
test('Nested Context', async (subtest) => {
  ...
  await subtest.test('Level_1', async (tst) => {
    ...
    await tst.test('#compileValidationSchema', async (ntst) => {
      ...
      await ntst.test('Should reuse the validate fn across multiple invocations - Route without schema', async (t) => {
        const fastify = Fastify()
        let validate = null
        let counter = 0
      ...
      await ntst.test('Should use the custom validator compiler for the route', async (t) => {
        const fastify = Fastify()
        let called = 0
    ...
    await tst.test('#getValidationFunction', async (ntst) => {
      ...
      await ntst.test('Should return the validation function from each HTTP part', async (t) => {
        const fastify = Fastify()
        let headerValidation = null
        let customValidation = null
      ...
      await ntst.test('Should return undefined if no schema compiled - nested', async (t) => {
        const fastify = Fastify()
        let called = 0
        const custom = ({ schema, httpPart, url, method }) => {
          called++
    ...
    await tst.test('#validate', async (ntst) => {
      ...
      await ntst.test('Should return true/false if input valid - Route without schema', async (t) => {
        const fastify = Fastify()
      ...
      await ntst.test('Should use the custom validator compiler for the route', async (t) => {
        ...
        fastify.register((instance, opts, next) => {
          instance.get('/', { validatorCompiler: customChild }, (req, reply) => {
            const ok = req.validateInput({ hello: 'world' }, defaultSchema, 'querystring')
            const ok2 = req.validateInput({ hello: 'world' }, defaultSchema)
