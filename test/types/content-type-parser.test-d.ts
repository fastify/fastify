...
expectType<void>(
  fastify().addContentTypeParser('contentType', function (request, payload, done) {
...
expectType<void>(
  fastify().addContentTypeParser('contentType', { bodyLimit: 99 }, function (request, payload, done) {
    expectType<FastifyRequest>(request)
    expectType<IncomingMessage>(payload)
    done(null)
  })
...
// Body Parser - the generic after addContentTypeParser enforces the type of the `body`
// parameter as well as the value of the `parseAs` property

expectType<void>(
  fastify().addContentTypeParser<string>('bodyContentType', { parseAs: 'string' }, function (request, body, done) {
    expectType<FastifyRequest>(request)
...
expectType<void>(
  fastify().addContentTypeParser<Buffer>('bodyContentType', { parseAs: 'buffer' }, function (request, body, done) {
    expectType<FastifyRequest>(request)
...
expectType<void>(
  fastify().addContentTypeParser('contentType', async function (request: FastifyRequest, payload: IncomingMessage) {
    expectType<FastifyRequest>(request)
    expectType<IncomingMessage>(payload)
    return null
  })
