...
expectAssignable<PromiseLike<string>>(server.listen({ port: 3000, listenTextResolver: (address) => {`address: ${address}`} }))
...
expectAssignable<void>(server.listen({ port: 3000, listenTextResolver: (address) => {`address: ${address}`} }, () => {}))
