...
> {
  ...
  */
   ...
   * Set the 404 handler
   */
  setNotFoundHandler<RequestGeneric extends RequestGenericInterface = RequestGenericInterface, ReplyPayload = unknown>(
    handler: (request: FastifyRequest<RequestGeneric, RawServer, RawRequest>, reply: FastifyReply<ReplyPayload, RawServer, RawRequest, RawReply>) => void
  ): void;

  /**
   * Set a function that will be called whenever an error happens
   */
  setErrorHandler<TError extends Error = FastifyError, RequestGeneric extends RequestGenericInterface = RequestGenericInterface, ReplyPayload = unknown>(
    handler: (this: FastifyInstance<RawServer, RawRequest, RawReply, Logger>, error: TError, request: FastifyRequest<RequestGeneric, RawServer, RawRequest>, reply: FastifyReply<ReplyPayload, RawServer, RawRequest, RawReply>) => void
  ): void;
