...
export interface FastifyListenOptions {
   ...
   * The actual length will be determined by the OS through sysctl settings
   * such as `tcp_max_syn_backlog` and `somaxconn` on Linux.
   * Default to `511`.
   */
  backlog?: number
  /**
...
type DecorationMethod<This, Return = This> = {
  <
    // Need to disable "no-use-before-define" to maintain backwards compatibility, as else
    // decorate<Foo> would suddenly mean something new
...
> {
  ...
  // should be able to define something useful with the decorator getter/setter pattern using
  // Generics to enforce the users function returns what they expect it to
   ...
   *  Notice: in the `onRequest` hook, request.body will always be null, because
   ...
   * The onResponse hook is executed when a response has been sent, so you will not be able
   * to send more data to the client. It can however be useful for sending data to external
   * services, for example to gather statistics.
   */
  addHook<
    RouteGeneric extends RouteGenericInterface = RouteGenericInterface,
    ContextConfig = ContextConfigDefault,
   ...
   * The `onRequestAbort` hook is executed when a client closes the connection before the entire
   * request has been received. Therefore, you will not be able to send data to the client.
   ...
   * the customErrorHandler sends an error back to the user
   * (Note that the default customErrorHandler always sends the error back to the user).
   ...
   * It is useful when plugins need a "onListen" event, for example
   * to run logics after the server start listening for requests.
   */
  addHook<
    Fn extends
